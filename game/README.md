...
- `Assets/Scripts/` - all game code, built scene-free: `Bootstrap` (Boot scene entry), `GameManager` (front-end + fight flow), `RoundManager` (best-of-3, banners, Khulandra event, KO), `PlayerController` / `EnemyAI` / `Fighter` / `CombatSystem` (deterministic fight sim), `StageManager` (parallax layers + flooded swap), `TouchInput` (tap/swipe/hold gestures), `TouchUI` / `MainMenu` / `StoryIntro` / `HowToPlay` / `FightSelectMenu` / `EndingPanel` (all UI in code), `GltfCharacterLoader` + `FighterAnimator` (glTFast runtime GLB + clip playback), `AudioManager`, `AssetLib`, `UiKit`.
