...
*The Unity project is scaffolded and code-complete for the vertical slice (37 C# scripts in `game/`). This doc records the stack (fixed by VISION.md §11), the layout, the code↔asset contract, the animation pipeline (D-011), and the multiplayer/networking seam (D-017).*
...
- `game/` — the Unity project (scaffolded 2026-07-18 from DESIGN_BRIEF.md).
  - `game/Assets/Scripts/` — 37 C# files, everything built code-first from a near-empty Boot scene (no hand-authored prefabs). Includes the multiplayer front-end + networking seam (D-017), the onboarding front end (D-022), the distance/spacing layer (D-023), the moveset + guard-stance layer (D-024), and the selective slow-motion layer (D-026).
...
- **`Time.timeScale` has exactly one owner** (**D-026**). `TimeDirector` writes it; nothing else may, and `GameManager.TogglePause` routes through `TimeDirector.SetPaused` (pause wins, and cancels any running cinematic rather than layering on it). This is a change of position from the `CombatFx` header, which avoided `timeScale` *because* pause owned it — the resolution is single ownership, not avoidance. Slow motion uses `timeScale` deliberately: every deadline in the fight is an absolute `Time.time` stamp and every movement integrates `Time.deltaTime`, so one write dilates recovery, stun, walk, the legacy rig clip, the `ProcAnim` envelope and the VFX fades **in step** — a hand-rolled dilation layer would have to touch every script and would drift the moment one was missed. **Hit-stop keeps its separate freeze-window pattern** and composes: a cinematic's lead phase waits for `CombatFx` to unfreeze before ramping, so the two never contend for the same frame. Two standing rules for anything added later: a coroutine that sequences *around* a cinematic must wait in **unscaled** time (`TimeDirector.WaitForRelease`) or it will be stretched by the effect it is waiting on; and HUD chrome stays on `unscaledDeltaTime` so the interface never feels like it is lagging with the fight.
- **Cinematic policy is separate from the cinematic effect** (**D-026**). `TimeDirector` = how a shot looks (envelope, curves, audio coupling, camera dolly); `Cinematics` = which hits earn one (triggers + the per-round budget). The two halves of the brief are independent problems, so the rarity can be re-tuned without touching a curve and vice versa. Every trigger is **deterministic** — no RNG — both because a random crit would make the same read look different on two identical inputs, and to keep the sim lockstep-able for D-017. `Cinematics.OnlineLockout` disables the whole layer under `Backend.Relay`, since dilating one peer's clock is a desync.
